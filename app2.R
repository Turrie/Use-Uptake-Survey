...
#setwd("C:/Users/User/Documents/ACAI/UseUptake3")
source("global.R")
...
ui <- fluidPage(
  ...
  navbarPage(title=div(img(src = "images/akilimo_icon.png", height = "40px", style = "position: relative; top: -3px; left: -1000px;"),"AKILIMO Tools Use and Uptake Survey" ),
          ...
          tabPanel("Overall",
                   ...
                   fluidRow(
                     box(title = NULL, status = "primary", id = "box_use_uptake_ng", 
                         #title = tags$p("Use and uptake in Nigeria", style = "font-size: 16px"), 
                     ...
                     box(id = "box_use_uptake_tz", 
                         #title = tags$p("Use and uptake in Tanzania", style = "font-size: 16px"), 
                         fluidRow(column(width = 6,
                   ...
                   absolutePanel(id = "logo", class = "card", top = 40, right = 50, width = 100, fixed=TRUE, draggable = FALSE, 
                                 tags$img(src='images/akilimo_icon.png',height = 150, width = 150
                                 ),
                                   ...
                                   sidebarLayout(
                                     sidebarPanel(width = 2,
                                       ...
                                       conditionalPanel(
                                         ...
                                         #&& input.country_sel != 'Compare countries'", 
                                         ...
                                         selectInput("group_sel1_0", label = h5(tags$p("Grouping variable", style = "margin-left: 16px; font-size: 16px")),
                                                     choices = subgroups_all, selected = "genderHH"),
            ...
            absolutePanel(id = "logo", class = "card", top = 35, right = 50, width = 100, fixed=TRUE, draggable = FALSE, 
                          tags$img(src='images/akilimo_icon.png',height = 150, width = 150
                          ),
                          onclick = sprintf("window.open('%s')",
                                            "https://akilimo.org")
...
server <- function(input, output,session) {
    ...
    if(input$country_sel == "Compare countries") {
      ...
      fluidRow(
        box(title = textOutput("title_by_covariates"), id = "box_covariates",
            status = "primary", solidHeader = F, collapsible = F, width = 12,
            plotlyOutput("use_uptake_gh", height = "auto", width = "1000px")
        )
      ) 
  ...
  analysisData <- reactive({
    ...
    } else if(selected_grouping_var == "use_case") {
      
      analysis_dataset <- ds1 %>%
        dplyr::select(unique(c(filter_vars, selected_facet_var)), use_uptake_var, FR, IC, PP_WM, SPHS) %>%
        rename("use_or_uptake" = use_uptake_var) %>%
        gather(GrpVar, Value, FR:SPHS) %>%
        mutate(GrpVar = factor(GrpVar, levels = c( "FR", "IC", "PP_WM", "SP_HS"), labels = c( "FR", "IC", "PP/WM", "SP/HS"))) %>%
        filter(Value) %>%
  ...
  output$ng_respondents <- renderValueBox({
    ...
    valueBox(value = tags$p(div(img(src = 'images/ng.png', height = '50px', style = 'font-size: 16px', br(),br(),
  ...
  output$tz_respondents <- renderValueBox({
    ...
    valueBox(value = tags$p( div(img(src = "images/tz.PNG", height = "50px", style = "font-size: 16px", br(),br(),
  ...
  output$use_uptake_gh <- renderPlotly({
    ...
    ## filter by country and partner only
    else if(country_partner_sel) {
    ...
    ## filter by country and partner and disaggregate by variable
    else if(all_sel1_0) {
        ...
        observeEvent(event_map_tz(),
                     {
                       ...
                       output$event_plt <- renderTmap({
                         ...
                         output$event_plt <- renderTmap({
                           #tmap_mode("view")
                           TZ_events <- tm_shape(event_map_tz()) +
                             tm_polygons(col = "freq", breaks = c(0,10,20,30,60,90,100,200,500) , palette = "Greys", direction=-1, title = paste("Number of", " ",  selection, sep = ""))+
                             tm_text(text = "NAME_1")
                           TZ_events
                         ...
                         output$event_plt <- renderTmap({
                           #tmap_mode("view")
                           TZ_events <- tm_shape(event_map_tz()) +
                             tm_polygons(col = "freq", breaks = c(0,10,20,30,60,90,100,200,500) , palette = "Purples", direction=-1, title = paste("Number of", " ",  selection, sep = ""))+
                             tm_text(text = "NAME_1")
                           TZ_events
                         ...
                         output$event_plt <- renderTmap({
                           #tmap_mode("view")
                           TZ_events <- tm_shape(event_map_tz()) +
                             tm_polygons(col = "freq", breaks = c(0,10,20,30,60,90,100,200,500,1000) , palette = "Oranges", direction=-1, title = paste("Number of", " ",  selection, sep = ""))+
                             tm_text(text = "NAME_1")
                           TZ_events
      ...
      if(input$country=="Nigeria"){
        output$event_plt <- renderTmap({
          NG_participants <- tm_shape(NGReg) +
            tm_polygons(col = "total", breaks = c(0,200,500,1000, 2000, 3000, 5000, 10000, 30000, 40000, 60000),
                        palette = "Greens", direction=-1, title = selection)+
            tm_text(text = "NAME_1") 
          NG_participants
        ...
        output$event_plt <- renderTmap({
          TZ_participants <- tm_shape(TZReg) +
            tm_polygons(col = "total", breaks = c(0,200,500,1000, 2000, 3000, 5000, 10000, 30000, 40000, 60000),
                        palette = "Blues", direction=-1, title = selection)+
            tm_text(text = "NAME_1")+
  ...
  output$event_plotly_TZ <- renderPlotly({
    ...
    cols <- c("#33ABB9", "#6850A0", "#609E25", "#FC8D62", "#CF6E64", "#FFC30F", "grey")
   # "#CF6E64" "#C0D64B" "#E69F00" "#2A6D8E" "#33ABB9" "#000000"
    names(cols) <- c("Agricultural show", "Field day", "Training event", "Media event", "Video show",
                     "Sensitization event", "Demonstration field")
  ...
  output$use_plotly <- renderPlotly({
    cols <- c("#f46d43", "#9dd2bf", "#ffd96a", "#1a9850")
  ...
  output$part_usecase_TZ <- renderPlotly({
    ...
    pt_use_TZ <- ggplot(usept_ds_TZ, aes(x=id)) +
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format()) +
       labs(
        title=paste("Participants by use case (TZ)"),
        x='', y=''
      ) +
      theme_classic() +
      theme(legend.position='top')
  ...
  output$part_usecase_NG <- renderPlotly({
   ...
   #spectral ,
    ...
    cols <- c( "#CED6B0" ,"#F0CE9E", "#E19363", "#CA562C", "#7DAB98", "#008080", "#B1B3B3", "#989B9C" ,"#73737F")
    #4 grey cols <- c( "#CED6B0" ,"#F0CE9E", "#E19363", "#CA562C", "#7DAB98", "#008080", "#B1B3B3", "#989B9C" ,"#73737F")
   #1 cols <- c( "#B56D4E",  "#FCE269","#02818A" ,"#016C59" ,"#CC79A7", "#C7B6C8","#F28E47", "#927754", "#875742")
              # "#C7B6C8","#F28E47",  "#7783C4","#74B6BA")
    ...
    pt_use_NG <- ggplot(usept_ds_NG, aes(x=id)) +
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format()) +
        labs(
        title=paste("Participants by use case (NG)"),
        x='', y=''
      ) +
      theme_classic() +
      theme(legend.position='top')
  ...
  output$event_pt_tz <- renderPlotly({
    ...
    pt_event_tz <- ggplot(eventpt_ds_TZ, aes(x=Event)) +
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format()) +
       labs(
        title=paste("Participants by events (TZ)"),
        x='', y=''
      ) +
      theme_classic() +
      theme(legend.position='top')
    
    #ggplotly(pt_event_tz)
  ...
  output$event_pt_ng <- renderPlotly({
    ...
    pt_event_ng <- ggplot(eventpt_ds_NG, aes(x=Event))+
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format())+
        labs(
        title=paste("Participants by events (NG)"), x='', y='')+
  
      
      theme_classic()+
      theme(legend.position='top')
    
    #ggplotly(pt_event_ng)
  ...
  output$prtnr_pt_ng <- renderPlotly({
    ...
    pt_prtnr_ng <- ggplot(reach_prt_ng, aes(x=Partner))+
     ...
     # scale_fill_brewer(palette='Dark2', direction=1)+
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format())+
      labs(
        title=paste("Participants by partners (NG)"),
        x='', y='')+
  ...
  output$prtnr_pt_tz <- renderPlotly({
    ...
    pt_prtnr_tz <- ggplot(reach_prt_tz, aes(x=Partner)) +
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format()) +
      labs(
        title=paste("Participants by partners (TZ)"),
        x='', y=''
      ) +
      theme_classic() +
      theme(legend.position='top')
  ...
  g_NG <- ggplot(gender_ds_NG, 
    ...
    scale_y_continuous(expand=expansion(0), labels=scales::percent_format()) +
    labs(
      title=paste("Attendees by gender and event (NG)"),
      x='', y=''
    ) +
    coord_flip()+
  ...
  output$gen_TZ <- renderPlotly({
    ...
    g_TZ <- ggplot(gender_ds_TZ, 
      ...
      scale_y_continuous(expand=expansion(0), labels=scales::percent_format()) +
      labs(
        title=paste("Attendees by gender and event (TZ)"),
        x='', y=''
      ) +
      coord_flip()+
  ...
  output$partnr_plotly <- renderPlotly({
    ...
    prtnr <- ggplot(parti_attendees2, aes(partner, total, 
                                          text = glue("Partner: {partner}
                                                       Total: {total}"))) +   
      geom_bar(
        stat = "identity", fill="#1a9850", show.legend = FALSE)+
      facet_grid(country ~ .)+
      labs(
        title=paste("Total number of attendees by partners"), x='', y='') +
