...
ui <- fluidPage(
  ...
  navbarPage(title=div(img(src = "https://akilimo.sirv.com/images/akilimo_icon.png", height = "50px", style = "position: relative; top: -3px; left: -1000px;"),"Akilimo Tools Use and Uptake Survey" ),
  ...
  # navbarPage(theme = shinytheme("yeti"), collapsible = TRUE,inverse = "FALSE",
  #         # title = div("Akilimo Tools Use and Uptake Survey", img(src = "https://akilimo.sirv.com/images/akilimo_icon.png", height = "90px", 
  ...
  #            #title = div(img(src='https://akilimo.sirv.com/images/akilimo_icon.png',style="margin-top: -24px; right: -1000px;", height = 80), "Tools Use and Uptake Survey"),
  #            "Tools Use and Uptake Survey", id="nav",
             #darkly,cyborg,flatly, slate, cosmo, yeti,readabl, sandstone, simplex
             #"Akilimo Tools Use and Uptake Survey", id="nav",
          tabPanel("Overall",
...
server <- function(input, output,session) {
  ...
  output$ng_respondents <- renderValueBox({
    ...
    valueBox(value = tags$p(div(img(src = 'https://akilimo.sirv.com/images/ng.png', height = '50px', style = 'font-size: 16px', br(),br(),
             ...
             color = "green")
    # valueBox(
    #   value = tags$p(paste0(format(ng_no$n, big.mark = ',', scientific = FALSE), " (", scales::percent(ng_fperc$Percent)," female)"), div(img(src = "https://akilimo.sirv.com/images/ng.png", height = "30px", style = "font-size: 16px"))),
  ...
  output$event <- renderUI({
    ...
    selectInput("event", "Select event to view by:",
                c("Agricultural show", "Training event", "Demonstration field",
                  "Media event", "Video show","Field day","Sensitization event"),
        ...
        observeEvent(event_map_tz(),
                     {
                       ...
                       output$event_plt <- renderTmap({
                         ...
                         TZ_events <- tm_shape(event_map_tz()) +
                           tm_polygons(col = "freq", breaks = c(0,10,20,30,60,90,100,200,500) , palette = "Set2", direction=-1, title = paste("Number of", " ",  selection, sep = ""))+
                           tm_text(text = "NAME_1")
                         TZ_events
        ...
        output$event_plt <- renderTmap({
          NG_participants <- tm_shape(NGReg) +
            tm_polygons(col = "total", breaks = c(0,200,500,1000, 2000, 3000, 5000, 10000, 30000, 40000, 60000),
                        palette = "Set1", direction=-1, title = selection)+
            tm_text(text = "NAME_1") 
          NG_participants
